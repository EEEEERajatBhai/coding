...
int main() {
    ...
    for (index = 0; index < no; index++) {
        scanf(" %c", &array1[index]);  // Use " %c" to consume any leftover whitespace
    }
    array1[no] = '\0';  // Null-terminate array1

    printf("Enter the elements of array2:\n");
    for (index = 0; index < no; index++) {
        scanf(" %c", &array2[index]);  // Use " %c" to consume any leftover whitespace
    }
    array2[no] = '\0';  // Null-terminate array2
